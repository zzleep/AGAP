<template>
  <div class="space-y-4">
    <div>
      <h2 class="text-xl font-bold text-white">{{ $t('communityReport.title') }}</h2>
      <p class="text-xs text-slate-400">{{ $t('communityReport.description') }}</p>
    </div>
    ...
    <div class="p-3 rounded-lg bg-slate-800/80 border border-slate-700/80 text-xs text-slate-300 flex items-start space-x-2">
      ...
      <div>
        <span class="font-bold text-white block">{{ $t('communityReport.anonymityTitle') }}</span>
        {{ $t('communityReport.anonymityDescription') }}
      </div>
    </div>
    ...
    <form @submit.prevent="handleInitialSubmit" class="p-4 rounded-xl bg-slate-800 border border-slate-700 space-y-4 shadow-md">
      <!-- Barangay Dropdown -->
      <div>
        <label class="block text-xs font-semibold text-slate-300 mb-1">{{ $t('communityReport.selectBarangay') }}</label>
        <select
      ...
      <div>
        <label class="block text-xs font-semibold text-slate-300 mb-1">{{ $t('communityReport.incidentDescription') }}</label>
        <textarea
      ...
      <div v-if="submissionSuccess && submittedData" class="p-4 rounded-xl bg-emerald-950/70 border border-emerald-700 text-xs space-y-2">
        ...
        <div class="text-slate-300 space-y-1 text-[11px]">
          ...
          <p><strong>{{ $t('communityReport.aiCategory') }}</strong> <span class="capitalize text-blue-300 font-semibold">{{ submittedData.ai_category || $t('communityReport.triagePending') }}</span></p>
          <p><strong>{{ $t('communityReport.aiPriority') }}</strong> <span class="capitalize text-amber-300 font-semibold">{{ submittedData.ai_priority || $t('communityReport.medium') }}</span></p>
    ...
    <div v-if="showNagModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm">
      <div class="max-w-sm w-full p-5 rounded-xl bg-slate-800 border border-amber-500/50 shadow-2xl space-y-4">
        <div class="flex items-center space-x-2 text-amber-400">
          ...
          <h3 class="font-bold text-base text-white">{{ $t('communityReport.nagTitle') }}</h3>
        </div>
